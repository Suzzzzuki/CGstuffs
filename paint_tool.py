...
def draw_circle(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if drawing == True:
            cv2.circle(img, (x, y), g_size, g_color, -1)
            # Draw circle using cv2.circle function where g_size is radius and g_color is color
...
while(1):
    ...
    if k in color_pallet: # IF statement to determine if k is a key for color_pallet
        g_color = color_pallet[k]
    if k in pen_size: # ELSE IF statement to determine if k is a key for pen_size
        g_size = pen_size[k]        
    elif k == ord('q'):
