...
class Spotlights:
    ...
    def calcColorFromLight(self, target_pos):
        ...
        if math.acos(np.dot(L2T, light_direction)) <= self.phi/2: ## The condition formula that the angle between L2T and light_direction is less than or equal to phi/2 ##
            colored_by_light = Lambert*self.I0*np.array(self.color)/distance**2

            #assign3
            V = (np.array(cam_pos) - target_pos)/np.linalg.norm(np.array(cam_pos) - target_pos)
            N = N/np.linalg.norm(N)
            R = L2T/np.abs(np.dot(L2T, N)) + 2*N
            R2 = R/np.linalg.norm(R)
            colored_by_light += self.I0*(np.dot(R2, V)**10)

        if (math.acos(np.dot(L2T, light_direction)) <= self.theta/2) & (math.acos(np.dot(L2T, light_direction)) > self.phi/2):## The condition formula that the angle between L2T and light_direction is less than or equal to theta/2 and more than phi/2 ##
...
def main():
    ...
    global spotlight
    light_pos = np.array([0.0, 2.0, 0.0])
    color1 = [1.0, 0.0, 0.0]
